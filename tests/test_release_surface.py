...
def test_ci_workflow_covers_manifest_inputs_and_published_consumer_path():
    ...
    assert any(step.get("uses") == "yeaight7/dbt-semguard@v0.4.0" for step in published_steps)
    assert any(step.get("uses") == "yeaight7/dbt-semguard@v0.4.0" for step in published_manifest_steps)
